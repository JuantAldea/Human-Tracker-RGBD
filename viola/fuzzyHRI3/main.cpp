...
#define MAXIMUMPERSONS 10 //maximum number of people that can be tracked at the same time
#define MOUTHCENTER 0.1 //We consider that the mouth center should be 10% left/right and up/down from the down half of the face square
#define ERRORDISTANCEMETERS 0.25 // we consider this the maximum error distance in meters which to consider that an openCV face belongs to the tracked pers
#define ERRORDISTANCEPIXELS 20 // we consider this the maximum error distance in pixels which to consider that an openCV face belongs to the tracked pers
...
#define UMBRALDISTANCEGESTURE 0.9 //distance of the pixel to the center of person to consider that it could belong to the extending arm
#define MINDISTANCE 0.5 //minimum distance of person to camera
#define MAXDISTANCE 5.0 //maximum distance of person to camera
#define DISTANCEPOINT1 1.0 // distance where close distance starts to decrease and far distance starts to increase
#define DISTANCEPOINT2 1.6 //distance where close distance ends and fardistance is at maximum
#define LEARNINGRATEINTEREST 0.3 //learning rate for final interest value

int handle_OpenCV_error(int status, const char* func_name, const char* err_msg,
...
string uso()
{
    return "This program allows to track one or several people using plan view maps. You must indicate the following parameters.\n \t StereoVideo \n\t Num Frames Background: number of initial frames employed to create background \n\t Camera Height \n\t Camera tilt angle\n";
}
...
//actions to perform when button is pressed in the window associated to the events function
{

    CallbackData *callback_data = (CallbackData*) data;
    /////////////////////////////
    //Adds people position to the list
    if (event == CV_EVENT_LBUTTONDOWN && x >= callback_data->StImage->getWidth()) {
...
//
/////////////////////////////////////////////
void createTransformMatrix(float PtuHeight, float PtuPan, float PtuTilt,
                           _3DTransformationMatrix &Matrix)throw(GUException)
{
    ...
    TMatrix[0][3] = (cos(panRadians) * dax + day * sin(panRadians) * sin(tiltRadians));
    ...
    TMatrix[2][3] = -dax * sin(panRadians) + day * sin(tiltRadians) * cos(panRadians);
...
int main(int argc, char **argv)
{
    ...
    long numberDifferentBinaryPixels, numberDifferentRedPixels, numberDifferentBluePixels,
         numberDifferentGreenPixels, numberDifferentDistancePixels, numberOfPixels;
    vector<float> inRAP, inRAM, outRAP, outRAM, inCloseFS, outCloseFS, inFarFS, outFarFS;
    ...
    //HaarObjectDetector FaceDetector("/usr/local/share/OpenCV/haarcascades/haarcascade_frontalface_default.xml");
    //cv::CascadeClassifier FaceDetector1 ("/usr/local/share/OpenCV/haarcascades/haarcascade_frontalface_default.xml");
    ...
    try {
        ...
        ///Create 3D transform matrix to move 3D points to a reference system in the floor
        ...
        StImage = VideoReader.produceNext()->get(0); //retrieves the first stereo image (number 0)
        PMaskExtractor.setParams(StImage->getWidth(), StImage->getHeight(), 120, 120);
        ///allocateIplImages (OpenCV images)
        rgbCameraImage = cvCreateImage(cvSize(StImage->getWidth(), StImage->getHeight()), 8, 3);
        toVideo = cvCreateImage(cvSize(StImage->getWidth(), StImage->getHeight()), 8, 3);
        ...
        closeFS.loadFromFile("modelos/closefuzzy.fis"); // Sistema Difuso para "close"
        farFS.loadFromFile("modelos/farfuzzy.fis"); // Sistema Difuso para "faraway"
        inRAP.resize(2);
        inRAM.resize(2);
        inCloseFS.resize(2);
        inFarFS.resize(3);
...
#define TODO
        ...
        ///Init background
        int nBackgrounFrames = atoi(argv[2]); //number of frames employed for background modelling
        ...
        for (int i = 0; i < nBackgrounFrames; i++) {
            ...
            StImage->colorImageToBGRBuffer((unsigned char *)(rgbCameraImage->imageData),
                                           StereoImage::REFERENCE);
            //now, convert to HSV
        ...
        cvInitFont(& font, CV_FONT_HERSHEY_TRIPLEX, 0.4f,
                   0.4f); //parameters of the font which is used to write in the openCV images
        //Now, create plan view maps and show it waiting until
        //the user press on the person to be tracked
        cout << "Person autodetection OFF, Press over the person to track in the \"planview\" window with the left button"
             << endl;
        ...
        while (!VideoReader.eof()) { //cicle while the video does not end
            ...
            IplImage *foreGround = Background.getForeground(hsvCameraImage, StImage->getDepthImage(),
            ...
            PVMaps.create((unsigned char*)hsvCameraImage->imageData, StImage->getDepthImage(),
                          StImage->getWidth(), StImage->getHeight(), (unsigned char*)binForegorund->imageData);
            //now, show the image
            PVMaps.createDisplayableColorMap((unsigned char*)rgbCameraImage->imageData,
                                             StImage->getDepthImage(), StImage->getWidth(), StImage->getHeight(), &colorMap, binForegorund);
            ImgComposer.createNormalComposedImage(PVMaps, rgbCameraImage, StImage->getDepthImage(),
                                                  foreGround, &Tracker);

            //FaceDetector.detect(rgbCameraImage); // Detecta a la cara de todas las personas en la imagen.
            //detectedFaces = FaceDetector.getFacePositions();
            toVideo = cvCloneImage(rgbCameraImage); // Haz una copia clone de la imagen
            xyz = StImage->getDepthImage(); // Calcula la imagen de profundidad de la imagen

            if (Tracker.getNumPeopleBeingTracked() != 0) { // Si hay personas a seguir...
                frame++;
                ...
                for (int person = 0; person < Tracker.getNumPeopleBeingTracked();
                    ...
                    if (firstTime[person]) { // Si es la primera vez que detectamos a esta persona no hay historia de sus posiciones... Decimos que todo es igual para los frames de historia.
                    ...
                    // Calculamos si la persona se ha movido ya que solo vamos a analisar algunas cosas si la persona no se mueve mucho...
                    speed = sqrt(pow(positionX[person][0] - positionX[person][FRAMESPERPERSON - 1],
                                     2) + pow(positionZ[person][0] - positionZ[person][FRAMESPERPERSON - 1], 2)) * 100.0;
                    ...
                    for (int icv = 0; icv < (detectedFaces ? detectedFaces->total : 0);
                        ...
                        cvRectangle(toVideo, cvPoint(rectFace->x, rectFace->y), cvPoint(rectFace->x + rectFace->width,
                                    rectFace->y + rectFace->height), cvScalar(0, 0, 255), 2); // vermelho
                        positionFaceX = xyz[(rectFace->y + rectFace->height / 2) * StImage->getWidth() * 3 +
                                            (rectFace->x + rectFace->width / 2) * 3];
                        positionFaceY = xyz[(rectFace->y + rectFace->height / 2) * StImage->getWidth() * 3 +
                                            (rectFace->x + rectFace->width / 2) * 3 + 1];

                        positionFaceZ = xyz[(rectFace->y + rectFace->height / 2) * StImage->getWidth() * 3 +
                        ...
                        if (dist < ERRORDISTANCEMETERS
                                && positionFaceY >
                                1.3) { // && abs(rectFace->width - (int)(96.0/personPos.second)) < 2.0/personPos.second*ERRORDISTANCEPIXELS && abs (rectFace->height - (int)(115.0/personPos.second)) < 4.0/personPos.second*ERRORDISTANCEPIXELS)
                            faceOpenCV =
                                true; //Para recuperar precision en el tracking, usamos cuando podemos la posicion de la cara de openCV y buscamos a sus cordenadas
                            if (dist < minDist) {
                    ...
                    if (faceOpenCV) {
                        rectFace = (CvRect*) cvGetSeqElem(detectedFaces,
                                                          goodHead); // Preparacion de la imagen de la cara para darle al detector de sonrisa
                        rectAux = cvRect(rectFace->x, rectFace->y, rectFace->width, rectFace->height);
                        ...
                        smileClass = SmileEstimatorForFace.estimate(smallFaceGrey); // Detector de sonrisa
                        ...
                        if (smileClass == -1) {
                            cout << "No smile detected for person " << person << endl;    // No hemos detectado sonrisa
                        }

                        cvRectangle(toVideo, cvPoint(headx, heady), cvPoint(headx + headWidth, heady + headHeight),
                    ...
                    if (!faceOpenCV) { // Values come from tracker porque ninguna cara detectada por openCV parece corresponder a una persona que esta siendo seguida
                        if (PMaskExtractor.createMask(StImage, personPos, foreGround, 0.5, 2.3)) {
                            guopencv::Utilities::copyMakeBorder2(PMaskExtractor.getNormalizedColorMask(),
                                                                 ...
                                                                 0.4);
                            guopencv::Utilities::copyMakeBorder2(PMaskExtractor.getColorMask(),
                    ...
                    case 1: //High Attention
                        attentionLevel[person] = attentionLevel[person] * (1 - LEARNINGRATEATTENTION) +
                                                 LEARNINGRATEATTENTION; // Version con "learning rate" por lo de tener en cuenta la historia. Si no nos interesa hay que desactivar.
                        break;
                    case 2: //Low Attention
                        attentionLevel[person] = attentionLevel[person] * (1 -
                                                 LEARNINGRATEATTENTION); // Version con "learning rate" por lo de tener en cuenta la historia. Si no nos interesa hay que desactivar.
                        break;
                    }
                    ...
                    if (analyseGestures[person]) { // solo analisamos el movimiento de los brazos si la persona no se esta moviendo (o casi nada)
                        cout << "IS NOT MOVING (A LOT) AND ATTENT" << endl;
                        numberOfPixels = numberDifferentBinaryPixels = numberDifferentRedPixels =
                                             numberDifferentBluePixels = numberDifferentGreenPixels = numberDifferentDistancePixels =
                                                         numberOfArmPixelsBinary = numberOfArmPixelsDistance = numberOfArmPixelsBinaryHistory =
                                                                 numberOfArmPixelsDistanceHistory = 0;
                        for (int line = 0; line < binaryMask->height; line++) { // numero de lineas de la imagen...
                            for (int x = 0; x < FRAMESPERPERSON;
                                    x++) { // numero de frames pasados que tomamos en cuenta (5 en este caso)
                                oldptrbinary[person][x] = (unsigned char *)(oldBinaryMask[person][x]->imageData +
                                                          oldBinaryMask[person][x]->widthStep * line);
                            ...
                            for (int column = 0; column < binaryMask->width;
                                ...
                                // vamos a ver cuantos pixels que apartienen a un brazo(s) hay delante la persona para el frame actual (a una distancia maxima como si los brazos se estuvieran moviendo delante la persona)
                                if ((float)(*oldptrdistance[person][0]) != 0) { // current distance image
                                    if (((float)((column - CENTERBINARYPICTUREX) * (column - CENTERBINARYPICTUREX)) / (float)(
                                                GESTUREMAXIMUMX * GESTUREMAXIMUMX)) + ((float)((-line - CENTERBINARYPICTUREY) *
                                                        (-line - CENTERBINARYPICTUREY)) / (float)(GESTUREMAXIMUMY * GESTUREMAXIMUMY)) < 1.0) {
                                        if ((float)(*oldptrdistance[person][0]) > UMBRALDISTANCEGESTURE) {
                                            numberOfArmPixelsDistance++;
                                        }
                                    }
                                }

                                // vamos a ver cuantos pixels que apartienen a un brazo(s) hay en el entorno de la persona para el total de los 5 frames en memoria
                                tempBool = false;
                                for (int i = 0; i < FRAMESPERPERSON;
                                        i++) // numero de frames pasados que tomamos en cuenta (5 en este caso)
                                ...
                                // vamos a ver cuantos pixels que apartienen a un brazo(s) hay delante la persona para el total de los 5 frames en memoria (a una distancia maxima como si los brazos se estuvieran moviendo delante la persona)
                                tempBool = false;
                                ...
                                if (tempBool) {
                                    if (((float)((column - CENTERBINARYPICTUREX) * (column - CENTERBINARYPICTUREX)) / (float)(
                                        ...
                                        biggestDistance = 0;
                                        for (int i = 0; i < FRAMESPERPERSON; i++)
                                            if (((float)(*oldptrdistance[person][i])) > biggestDistance) {
                        ...
                        // 200 y 500 son umbrales que hemos calculado para brazo(s) parado(s) y brazo(s) moviendose
                        cout << "TOTAL PIXELS: " << numberOfPixels << " ARMS SIDE: " << numberOfArmPixelsBinary <<
                             " ARMS FRONT: " << numberOfArmPixelsDistance << " ARMS SIDE MOVE: " <<
                             numberOfArmPixelsBinaryHistory << " ARMS FRONT MOVE: " << numberOfArmPixelsDistanceHistory <<
                             endl;
                    ...
                    if (speed < SPEED) {
                        analyseGestures[person] =
                            true;    // si velocidad inferior a un limite entonces analisamos los gestos, sino no analisamos (porque quizas no se trata de un movimiento de brazos pero de todo el cuerpo)
                    } else {
                        analyseGestures[person] = false;
                    }

                    if (analyseGestures[person]) { // si es para analisar...
                        try { // preparamos los vectores de entrada del sistema difuso de analise de gestos
                        ...
                        movementArms[person] = movementArms[person] * (1 - LEARNINGRATEARMS) + LEARNINGRATEARMS *
                                               RA; // Version con "learning rate" por lo de tener en cuenta la historia. Si no nos interesa hay que desactivar.
                ...
                //Analyse occlusion: Compares person[i] to every other person if distance of the other person to the camera is smaller than person[i]'s distance. Counts those pixels (which occlude person[i])
                for (int i = 0; i < Tracker.getNumPeopleBeingTracked(); i++) {
                    ...
                    for (int line = 0; line < binaryMask->height; line++) {
                        for (int j = 0; j < Tracker.getNumPeopleBeingTracked(); j++) {
                            oldptrbinary[j][0] = (unsigned char *)(oldBinaryMask[j][0]->imageData +
                        ...
                        for (int column = 0; column < binaryMask->width; column++) {
                            ...
                            for (int j = 0; j < Tracker.getNumPeopleBeingTracked(); j++) {
                                if (i != j && positionZ[j][0] < positionZ[i][0]) {
                                    if (((int)(*oldptrbinary[i][0])) != 0 && ((int)(*oldptrbinary[j][0])) != 0) {
                                        occludedPixels[i]++;
                                    }
                                }
                            }
                ...
                //Compute person's interest. Uses the output of either a "far fuzzy system" or "close fuzzy system"" according to the distance of the current person.
                for (int i = 0; i < Tracker.getNumPeopleBeingTracked(); i++) {
                    cout << "Distance person " << i << " is " << positionZ[i][0] << " meters. Activated ";
                    if (positionZ[i][0] > MINDISTANCE
                            && positionZ[i][0] < DISTANCEPOINT1) { // La persona esta cerca de la camera...
                        ...
                        //finalInterest[i] = finalInterest[i]*(1-LEARNINGRATEINTEREST) + LEARNINGRATEINTEREST*outCloseFS[0]; // Version con "learning rate" por lo de tener en cuenta la historia. Si no nos interesa hay que desactivar.
                        finalInterest[i] = outCloseFS[0]; // salida Interest = salida FS Close
                    } else if (positionZ[i][0] > DISTANCEPOINT2
                               && positionZ[i][0] < MAXDISTANCE) { // La persona esta lejos de la camera...
                        inFarFS[0] = attentionLevel[i]; // entrada atencion para FS Faraway
                        inFarFS[1] = movementArms[i]; // entrada movimiento brazos para FS Faraway
                        inFarFS[2] = occludedPixelsRatio[i]; // entrada pixels occlusion para FS Faraway
                        farFS.process(inFarFS, outFarFS);
                        //finalInterest[i] = finalInterest[i]*(1-LEARNINGRATEINTEREST) + LEARNINGRATEINTEREST*outFarFS[0]; // Version con "learning rate" por lo de tener en cuenta la historia. Si no nos interesa hay que desactivar.
                        finalInterest[i] = outFarFS[0]; // salida Interest = salida FS Faraway
                    } else if (positionZ[i][0] > DISTANCEPOINT1
                               && positionZ[i][0] <
                               DISTANCEPOINT2) { // La persona esta entre cerca y lejos... utilisamos los 2 Sistemas Difusos...
                        ...
                        farFS.process(inFarFS, outFarFS);
                        pertenenciaLejos = (positionZ[i][0] - DISTANCEPOINT1) / (DISTANCEPOINT2 - DISTANCEPOINT1);
                        //finalInterest[i] = finalInterest[i]*(1-LEARNINGRATEINTEREST)+LEARNINGRATEINTEREST*((1-pertenenciaLejos)*outCloseFS[0] + pertenenciaLejos*outFarFS[0]); // Version con "learning rate" por lo de tener en cuenta la historia. Si no nos interesa hay que desactivar.
                        finalInterest[i] = (1 - pertenenciaLejos) * outCloseFS[0] + pertenenciaLejos *
                                           outFarFS[0]; // para distancias intermedias el peso de cada FS depende de la distancia a que la persona se encuentra
                    } else {
            ...
            /*  if (!AviWriter.isValid())
                ...
                if (saveVideo) AviWriter.addFrame((unsigned char*)toVideo->imageData,false);
            */
