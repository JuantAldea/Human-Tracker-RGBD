...
// events function
{
    ...
    if (event == CV_EVENT_LBUTTONDOWN && x >= callback_data->StImage->getWidth()) {
        ...
        float* height = (float*)(callback_data->PVMaps->hMap.hMap->imageData + callback_data->PVMaps->hMap.hMap->widthStep * y);
        height += x;
        ...
        while (!VideoReader.eof()) { // cicle while the video does not end
            ...
            for (int person = 0; person < Tracker.getNumPeopleBeingTracked();
                ...
                speed = sqrt(pow(positionX[person][0] - positionX[person][FRAMESPERPERSON - 1],
                                 2) + pow(positionZ[person][0] - positionZ[person][FRAMESPERPERSON - 1],
                                          2)) * 100.0;
                ...
                for (int icv = 0; icv < (detectedFaces ? detectedFaces->total : 0);
                    ...
                    positionFaceX = xyz[(rectFace->y + rectFace->height / 2) * StImage->getWidth() * 3 + (rectFace->x + rectFace->width / 2) * 3];
                    positionFaceY = xyz[(rectFace->y + rectFace->height / 2) * StImage->getWidth() * 3 + (rectFace->x + rectFace->width / 2) * 3 + 1];

                    positionFaceZ = xyz[(rectFace->y + rectFace->height / 2) * StImage->getWidth() * 3 + (rectFace->x + rectFace->width / 2) * 3 + 2];

                    dist = abs(positionFaceX - personPos.first);

                    if (dist < ERRORDISTANCEMETERS && positionFaceY > 1.3) { // && abs(rectFace->width -
                ...
                if (faceOpenCV) {
                    ...
                    switch (smileClass) {
                    case 1: // Low Smile
                        smileLevel[person] = smileLevel[person] * (1 - LEARNINGRATESMILE); // Version con "learning rate" por
                        // lo de tener en cuenta la
                        // historia. Si no nos interesa
                        // hay que desactivar.
                        break;
                    case 2: // High Smile
                        smileLevel[person] = smileLevel[person] * (1 - LEARNINGRATESMILE) + LEARNINGRATESMILE; // Version con "learning rate" por lo de
                ...
                if (!faceOpenCV) { // Values come from tracker porque ninguna cara
                    ...
                    if (PMaskExtractor.createMask(StImage, personPos, foreGround, 0.5,
                        ...
                        guopencv::Utilities::copyMakeBorder2(
                            ...
                            cvPoint(indexWidth + PMaskExtractor.getNormalizedColorMask()->width,
                                0),
                ...
                case 1: // High Attention
                    attentionLevel[person] = attentionLevel[person] * (1 - LEARNINGRATEATTENTION) + LEARNINGRATEATTENTION; // Version con "learning rate" por lo
                    // de tener en cuenta la historia. Si
                    // no nos interesa hay que desactivar.
                    break;
                case 2: // Low Attention
                    attentionLevel[person] = attentionLevel[person] * (1 - LEARNINGRATEATTENTION); // Version con "learning rate"
                ...
                if (analyseGestures[person]) { // solo analisamos el movimiento de
                    ...
                    numberOfPixels = numberDifferentBinaryPixels = numberDifferentRedPixels = numberDifferentBluePixels = numberDifferentGreenPixels = numberDifferentDistancePixels = numberOfArmPixelsBinary = numberOfArmPixelsDistance = numberOfArmPixelsBinaryHistory = numberOfArmPixelsDistanceHistory = 0;
                    for (int line = 0; line < binaryMask->height;
                        ...
                        for (int x = 0; x < FRAMESPERPERSON;
                            ...
                            // en este caso)
                            oldptrbinary[person][x] = (unsigned char*)(oldBinaryMask[person][x]->imageData + oldBinaryMask[person][x]->widthStep * line);
                            oldptrdistance[person][x] = (float*)(oldDistanceMask[person][x]->imageData + oldDistanceMask[person][x]->widthStep * line);
                        }
                        for (int column = 0; column < binaryMask->width;
                            ...
                            if (((int)(*oldptrbinary[person][0])) != 0) { // current binary image
                                numberOfPixels++;
                                if ((((float)((column - CENTERBINARYPICTUREX) * (column - CENTERBINARYPICTUREX)) / (float)(GESTUREMINIMUMX * GESTUREMINIMUMX)) + ((float)((-line - CENTERBINARYPICTUREY) * (-line - CENTERBINARYPICTUREY)) / (float)(GESTUREMINIMUMY * GESTUREMINIMUMY)) > 1.0) && (((float)((column - CENTERBINARYPICTUREX) * (column - CENTERBINARYPICTUREX)) / (float)(GESTUREMAXIMUMX * GESTUREMAXIMUMX)) + ((float)((-line - CENTERBINARYPICTUREY) * (-line - CENTERBINARYPICTUREY)) / (float)(GESTUREMAXIMUMY * GESTUREMAXIMUMY)) < 1.0) && line < 70) {
                            ...
                            if ((float)(*oldptrdistance[person][0]) != 0) { // current distance image
                                if (((float)((column - CENTERBINARYPICTUREX) * (column - CENTERBINARYPICTUREX)) / (float)(GESTUREMAXIMUMX * GESTUREMAXIMUMX)) + ((float)((-line - CENTERBINARYPICTUREY) * (-line - CENTERBINARYPICTUREY)) / (float)(GESTUREMAXIMUMY * GESTUREMAXIMUMY)) < 1.0) {
                ...
                if (speed < SPEED) {
                    analyseGestures[person] = true; // si velocidad inferior a un limite entonces analisamos
                ...
                if (analyseGestures[person]) { // si es para analisar...
                    ...
                    movementArms[person] = movementArms[person] * (1 - LEARNINGRATEARMS) + LEARNINGRATEARMS * RA; // Version con "learning rate" por lo de
                    // tener en cuenta la historia. Si no
                    // nos interesa hay que desactivar.
            ...
            for (int i = 0; i < Tracker.getNumPeopleBeingTracked(); i++) {
                ...
                for (int line = 0; line < binaryMask->height; line++) {
                    for (int j = 0; j < Tracker.getNumPeopleBeingTracked(); j++) {
                        oldptrbinary[j][0] = (unsigned char*)(oldBinaryMask[j][0]->imageData + oldBinaryMask[j][0]->widthStep * line);
                    }
            ...
            for (int i = 0; i < Tracker.getNumPeopleBeingTracked(); i++) {
                cout << "Distance person " << i << " is " << positionZ[i][0]
                     << " meters. Activated ";
                if (positionZ[i][0] > MINDISTANCE && positionZ[i][0] < DISTANCEPOINT1) { // La persona esta cerca de la camera...
                ...
                else if (positionZ[i][0] > DISTANCEPOINT2 && positionZ[i][0] < MAXDISTANCE) { // La persona esta lejos
                    // de la camera...
                ...
                else if (positionZ[i][0] > DISTANCEPOINT1 && positionZ[i][0] < DISTANCEPOINT2) { // La persona esta entre cerca y
                    ...
                    // por lo de tener en cuenta la historia. Si no nos interesa hay que
                    // desactivar.
                    finalInterest[i] = (1 - pertenenciaLejos) * outCloseFS[0] + pertenenciaLejos * outFarFS[0]; // para distancias intermedias
